...
export function BottomNav() {
  ...
  // 5 slots — os 2 primeiros à esquerda, slot central reservado pro FAB
  // (renderizado absolutamente posicionado), 2 últimos à direita. Manter
  // o slot do meio como `<div>` invisível com flex:1 garante que os 4
  ...
  }: { path: string; label: string; Icon: typeof Home; badge?: number }) => {
      ...
      >
        {active && (
          <span
            style={{
              ...
              width: 32, height: 3, background: '#FFC107', borderRadius: '0 0 6px 6px',
            }}
          />
        )}
        {active && (
          <span
            style={{
              ...
              transform: 'translateX(-50%)',
              width: 44, height: 32, background: 'rgba(255,193,7,0.1)', borderRadius: 10,
            }}
          />
        )}
        <span style={{ position: 'relative', display: 'flex', zIndex: 1 }}>
          ...
          {badge && badge > 0 && (
            <span
              style={{
                position: 'absolute', top: -5, right: -8, minWidth: 17, height: 17,
                background: '#FD7E14', color: '#fff', fontSize: 10, fontWeight: 800,
                borderRadius: 9, display: 'flex', alignItems: 'center', justifyContent: 'center',
                padding: '0 4px', border: `2px solid ${navBg}`,
              }}
            >
              {badge}
            </span>
        ...
        <span style={{
          fontSize: 9, fontWeight: active ? 700 : 500, letterSpacing: 0.3, zIndex: 1,
        }}>
          {label}
        </span>
      </button>
